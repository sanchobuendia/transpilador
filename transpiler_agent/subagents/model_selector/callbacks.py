...
ALLOWED_TOOL_NAMES = {"select_model_for_project_tool"}
...
def before_model_callback(
    ...
    **_: Any,
    ...
    sanitize_or_raise(
        ...
        "Conteudo suspeito bloqueado antes da chamada ao modelo do model_selector",
    )
    return None
...
def before_tool_callback(
    ...
    **_: Any,
    ...
    sanitized_args = sanitize_or_raise(
        ...
        f"Conteudo suspeito bloqueado antes da execucao da tool {tool.name} no model_selector",
    )
...
def after_tool_callback(
    ...
    **_: Any,
    ...
    validated = validate_tool_result(tool.name, sanitize_or_raise(
        ...
        f"Saida suspeita bloqueada apos tool {tool.name} no model_selector",
    ))
    if isinstance(validated, dict) and validated.get("status") == "success" and "model_id" not in validated:
        raise ValueError("select_model_for_project_tool deveria retornar model_id no resultado de sucesso.")
    return validated
...
def after_model_callback(
    ...
    **_: Any,
    ...
    return sanitize_or_raise(
        ...
        "Saida suspeita bloqueada apos resposta do modelo do model_selector",
    )
