...
ALLOWED_TOOL_NAMES = {"generate_project_from_context_tool"}
...
def before_model_callback(
    ...
    **_: Any,
    ...
    sanitize_or_raise(
        ...
        "Conteudo suspeito bloqueado antes da chamada ao modelo do project_generator",
    )
    return None
...
def before_tool_callback(
    ...
    **_: Any,
    ...
    sanitized_args = sanitize_or_raise(
        ...
        f"Conteudo suspeito bloqueado antes da execucao da tool {tool.name} no project_generator",
    )
    sanitized_args = hydrate_args_from_context(
        sanitized_args,
        tool_context or callback_context,
        ("spec_json", "blueprint_json", "plan_json", "model_selection_json"),
    )
...
def after_tool_callback(
    ...
    **_: Any,
    ...
    validated = validate_tool_result(tool.name, sanitize_or_raise(
        ...
        f"Saida suspeita bloqueada apos tool {tool.name} no project_generator",
    ))
    if isinstance(validated, dict) and validated.get("status") == "success":
        required = ("output_dir", "generated_files")
        missing = [field for field in required if field not in validated]
        if missing:
            raise ValueError(f"generate_project_from_context_tool deveria retornar: {missing}")
    return validated
...
def after_model_callback(
    ...
    **_: Any,
    ...
    return sanitize_or_raise(
        ...
        "Saida suspeita bloqueada apos resposta do modelo do project_generator",
    )
